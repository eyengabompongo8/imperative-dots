...
Item {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    let parsedModes = {};
                    for (let i = 0; i < data.length; i++) {
                        parsedModes[data[i].name] = data[i].availableModes || [];
                    }
                    window.monitorModes = parsedModes;
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        monitorsModel.append({
                            ...
                            mirrorOf: (data[i].mirrorOf && data[i].mirrorOf !== "none" && data[i].mirrorOf !== "") ? (data[i].mirrorOf === "0" ? "eDP-1" : data[i].mirrorOf) : ""
                        });
    ...
    function triggerApply() {
        ...
        if (monitorsModel.count === 1) {
            ...
            let luaObj = "output = '" + m.name + "'";
            if (m.disabled) {
                // Use dpms off dispatcher — disabled=true via eval crashes Hyprland
                Quickshell.execDetached(["hyprctl", "dispatch", "dpms", "off", m.name]);
                Quickshell.execDetached(["notify-send", "Display Update", "Turned off: " + m.name]);
                return;
            } else if (m.mirrorOf !== "") {
                luaObj += ", mode = 'preferred', position = 'auto', mirror = '" + m.mirrorOf + "'";
            } else {
                let physX = Math.round(m.uiX / window.uiScale);
                let physY = Math.round(m.uiY / window.uiScale);
                let modeStr = (m.resW === 0 || m.resH === 0 || !m.rate) ? "preferred" : (m.resW + "x" + m.resH + "@" + m.rate);
                luaObj += ", mode = '" + modeStr + "', position = '" + physX + "x" + physY + "', scale = " + m.sysScale + ", mirror = ''";
                if (m.transform !== 0) {
                    luaObj += ", transform = " + m.transform;
                }
            }
            ...
            Quickshell.execDetached(["hyprctl", "eval", "hl.monitor({ " + luaObj + " })"]);
            ...
            for (let i = 0; i < monitorsModel.count; i++) {
                ...
                rects.push({
                    ...
                    disabled: m.disabled, mirrorOf: m.mirrorOf
                });
            }
            ...
            for (let i = 0; i < rects.length; i++) {
                ...
                let luaObj = "output = '" + r.name + "'";
                if (r.disabled) {
                    // Use dpms off dispatcher — disabled=true via eval crashes Hyprland
                    dpmsOffNames.push(r.name);
                } else if (r.mirrorOf !== "") {
                    luaObj += ", mode = 'preferred', position = 'auto', scale = " + r.sysScale + ", mirror = '" + r.mirrorOf + "'";
                    batchCmds.push("eval hl.monitor({ " + luaObj + " })");
                } else {
                    let modeStr = (r.resW === 0 || r.resH === 0 || !r.rate) ? "preferred" : (r.resW + "x" + r.resH + "@" + r.rate);
                    luaObj += ", mode = '" + modeStr + "', position = '" + r.x + "x" + r.y + "', scale = " + r.sysScale + ", mirror = ''";
                    if (r.transform !== 0) {
                        luaObj += ", transform = " + r.transform;
                    }
                    batchCmds.push("eval hl.monitor({ " + luaObj + " })");
                }
            ...
            // Fire dpms off for any disabled monitors first
            for (let i = 0; i < dpmsOffNames.length; i++) {
                Quickshell.execDetached(["hyprctl", "dispatch", "dpms", "off", dpmsOffNames[i]]);
            }

            // Apply the rest via batch
            if (batchCmds.length > 0) {
                let batchStr = batchCmds.join(" ; ");
            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                radius: window.s(8)
                                color: window.activeEditIndex === index ? window.surface1 : window.mantle
                                border.color: window.activeEditIndex === index ? window.selectedResAccent : window.surface0
                                border.width: window.activeEditIndex === index ? window.s(2) : window.s(1)
                                
                                Text {
                                    anchors.centerIn: parent
                                    font.family: "SF Mono"
                                    font.weight: window.activeEditIndex === index ? Font.Bold : Font.Normal
                                    font.pixelSize: window.s(12)
                                    color: window.activeEditIndex === index ? window.selectedResAccent : window.text
                                    text: model.name
                                }
                                MouseArea {
                                    anchors.fill: parent
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: window.activeEditIndex = index
                                }
                            }
                        }
                    }

                    // --- NEW: MONITOR MODE SELECTOR (Extend, Mirror, Off) ---
                    RowLayout {
                        id: modeSelector
                        Layout.fillWidth: true
                        ...
                        function updateStates() {
                            ...
                            isOff = m.disabled;
                            isMirror = !m.disabled && m.mirrorOf !== "";
                            isExtend = !m.disabled && m.mirrorOf === "";
                        }

                        Connections {
                            target: window
                            function onActiveEditIndexChanged() { modeSelector.updateStates(); }
                        }
                        Connections {
                            target: monitorsModel
                            function onDataChanged() { modeSelector.updateStates(); }
                        }
                        ...
                        function setMode(mode) {
                            ...
                            if (mode === "extend") {
                                monitorsModel.setProperty(idx, "disabled", false);
                                monitorsModel.setProperty(idx, "mirrorOf", "");
                                
                                let firstMonitor = null;
                                for(let i = 0; i < monitorsModel.count; i++) {
                                    if (i !== idx && !monitorsModel.get(i).disabled && monitorsModel.get(i).mirrorOf === "") {
                                        firstMonitor = monitorsModel.get(i);
                                        break;
                                    }
                                }
                                if (firstMonitor) {
                                    let isP = firstMonitor.transform === 1 || firstMonitor.transform === 3;
                                    let sW = (isP ? firstMonitor.resH : firstMonitor.resW) / firstMonitor.sysScale * window.uiScale;
                                    monitorsModel.setProperty(idx, "uiX", firstMonitor.uiX + sW + 10);
                                    monitorsModel.setProperty(idx, "uiY", firstMonitor.uiY);
                                }
                            } else if (mode === "mirror") {
                                monitorsModel.setProperty(idx, "disabled", false);
                                let target = (idx === 0 && monitorsModel.count > 1) ? monitorsModel.get(1).name : monitorsModel.get(0).name;
                                monitorsModel.setProperty(idx, "mirrorOf", target);
                            } else if (mode === "off") {
                                monitorsModel.setProperty(idx, "disabled", true);
                                monitorsModel.setProperty(idx, "mirrorOf", "");
                            }
                            delayedLayoutUpdate.restart();
                        }
                        ...
                        Repeater {
                            model: [
                                { label: "Extend", mode: "extend" },
                                { label: "Mirror", mode: "mirror" },
                                { label: "Off", mode: "off" }
                            ]
                            delegate: Rectangle {
                                Layout.fillWidth: true
                                ...
                                property bool isActive: {
                                    if (modelData.mode === "extend") return modeSelector.isExtend;
                                    if (modelData.mode === "mirror") return modeSelector.isMirror;
                                    if (modelData.mode === "off") return modeSelector.isOff;
                                    return false;
                                }

                                color: isActive ? Qt.alpha(window.selectedResAccent, 0.2) : window.surface0
                                border.color: isActive ? window.selectedResAccent : "transparent"
                                border.width: 1
                                ...
                                Text {
                                    ...
                                    font.family: "SF Compact"
                                    font.pixelSize: window.s(12)
                                    color: parent.isActive ? window.text : window.subtext0
                                    text: modelData.label
                                }
                                MouseArea {
                                    anchors.fill: parent
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: modeSelector.setMode(modelData.mode)
                                }
                            }
                        }
                    }
                    ...
                    Item { Layout.preferredHeight: window.s(2) }
                                        ...
                                        // --- NEW: DYNAMIC SETTINGS BOXES ---
                    property var currentAvailableModes: monitorsModel.count > 0 ? (window.monitorModes[monitorsModel.get(window.activeEditIndex).name] || []) : []
                    
                    property var availableResolutions: {
                        let resSet = new Set();
                        for (let m of currentAvailableModes) {
                            let res = m.split("@")[0];
                            resSet.add(res);
                        }
                    ...
                    property var availableRates: {
                        if (monitorsModel.count === 0) return [];
                        let currentRes = monitorsModel.get(window.activeEditIndex).resW + "x" + monitorsModel.get(window.activeEditIndex).resH;
                        let rateSet = new Set();
                        for (let m of currentAvailableModes) {
                            let parts = m.split("@");
                            if (parts[0] === currentRes && parts.length > 1) {
                                let r = parseFloat(parts[1].replace("Hz", ""));
                                rateSet.add(r);
                            }
                        }
                        return Array.from(rateSet).sort((a,b) => b-a);
                    }
                    ...
                    RowLayout {
                        ...
                        // Resolution Type-in box
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: window.s(4)
                            Text { text: "Resolution:"; color: window.subtext0; font.family: "SF Compact"; font.pixelSize: window.s(14) }
                        ComboBox {
                            id: resCombo
                            Layout.fillWidth: true
                            model: rightSideContainer.availableResolutions
                            editable: false
                            font.family: "SF Mono"
                            ...
                            background: Rectangle {
                                color: window.surface0
                                radius: window.s(6)
                                border.color: resCombo.activeFocus ? window.selectedResAccent : "transparent"
                            ...
                            popup: Popup {
                                ...
                                implicitHeight: contentItem.implicitHeight > window.s(150) ? window.s(150) : contentItem.implicitHeight
                                padding: 1
                                background: Rectangle {
                                    color: window.mantle
                                    border.color: window.surface0
                            ...
                            delegate: ItemDelegate {
                                ...
                                contentItem: Text {
                                    text: modelData
                                    color: highlighted ? window.selectedResAccent : window.text
                                    font.family: "SF Mono"
                                    font.pixelSize: window.s(14)
                                    verticalAlignment: Text.AlignVCenter
                                    horizontalAlignment: Text.AlignHCenter
                                ...
                                background: Rectangle {
                                    color: highlighted ? window.surface0 : "transparent"
                                }
                            }
                            ...
                            currentIndex: {
                                ...
                                let currentRes = monitorsModel.get(window.activeEditIndex).resW + "x" + monitorsModel.get(window.activeEditIndex).resH;
                                return rightSideContainer.availableResolutions.indexOf(currentRes);
                            }
                            onActivated: (index) => {
                                let text = model[index];
                                let parts = text.split("x");
                                if (parts.length === 2 && monitorsModel.count > 0) {
                                    monitorsModel.setProperty(window.activeEditIndex, "resW", parseInt(parts[0]));
                                    monitorsModel.setProperty(window.activeEditIndex, "resH", parseInt(parts[1]));
                                    delayedLayoutUpdate.restart();
                                }
                            }
                        }
                        ...
                        // Framerate Type-in box
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: window.s(4)
                            Text { text: "Framerate:"; color: window.subtext0; font.family: "SF Compact"; font.pixelSize: window.s(14) }
                        ComboBox {
                            id: rateCombo
                            Layout.fillWidth: true
                            model: rightSideContainer.availableRates
                            editable: false
                            font.family: "SF Mono"
                            font.pixelSize: window.s(14)
                            ...
                            background: Rectangle {
                                color: window.surface0
                                radius: window.s(6)
                                border.color: rateCombo.activeFocus ? window.selectedResAccent : "transparent"
                                border.width: window.s(1)
                            }
                            contentItem: Text {
                            ...
                            currentIndex: {
                                if (monitorsModel.count === 0) return 0;
                                let currentRate = parseFloat(monitorsModel.get(window.activeEditIndex).rate);
                                return rightSideContainer.availableRates.indexOf(currentRate);
                            }
                            onActivated: (index) => {
                                let val = model[index];
                                if (monitorsModel.count > 0) {
                                    monitorsModel.setProperty(window.activeEditIndex, "rate", val.toString());
                                }
                            }
                        }
                        }
                        ...
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: window.s(4)
                            Text { text: "Scale:"; color: window.subtext0; font.family: "SF Compact"; font.pixelSize: window.s(14) }
                        TextField {
                            id: scaleInput
                            Layout.fillWidth: true
                            font.family: "SF Mono"
                            ...
                            background: Rectangle {
                                color: window.surface0
                                radius: window.s(6)
                                border.color: scaleInput.activeFocus ? window.selectedResAccent : "transparent"
                                border.width: window.s(1)
                            }
                            ...
                            Connections {
                                ...
                                function onDataChanged(topLeft, bottomRight, roles) {
                                    if (window.activeEditIndex >= topLeft.row && window.activeEditIndex <= bottomRight.row) {
                                        if (!scaleInput.activeFocus) {
                                            scaleInput.text = monitorsModel.get(window.activeEditIndex).sysScale.toString();
                                        }
                                    }
                                }
                            }
                            ...
                            Connections {
                                ...
                                function onActiveEditIndexChanged() {
                                    if (!scaleInput.activeFocus && monitorsModel.count > 0) {
                                        scaleInput.text = monitorsModel.get(window.activeEditIndex).sysScale.toString();
                                    }
                                }
                            }
                            ...
                            onEditingFinished: {
                                if (monitorsModel.count > 0) {
                                    ...
                                    if (!isNaN(val) && val > 0) {
                                        monitorsModel.setProperty(window.activeEditIndex, "sysScale", val);
                                        delayedLayoutUpdate.restart();
                                    } else {
                                        // Reset to valid value
                                        text = monitorsModel.get(window.activeEditIndex).sysScale.toString();
                                    }
                                }
                            }
                        }
                        ...
                        // Position Type-in box
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: window.s(4)
                            Text { text: "Position:"; color: window.subtext0; font.family: "SF Compact"; font.pixelSize: window.s(14) }
                        TextField {
                            id: posInput
                            Layout.fillWidth: true
                            font.family: "SF Mono"
                            ...
                            background: Rectangle {
                                color: window.surface0
                                radius: window.s(6)
                                border.color: posInput.activeFocus ? window.selectedResAccent : "transparent"
                                border.width: window.s(1)
                            }
                            ...
                            Connections {
                                ...
                                function onDataChanged(topLeft, bottomRight, roles) {
                                    if (window.activeEditIndex >= topLeft.row && window.activeEditIndex <= bottomRight.row) {
                                        if (!posInput.activeFocus) {
                                            let mx = Math.round(monitorsModel.get(window.activeEditIndex).uiX / window.uiScale);
                                            let my = Math.round(monitorsModel.get(window.activeEditIndex).uiY / window.uiScale);
                                            posInput.text = mx + "x" + my;
                                        }
                                    }
                                }
                            }

                            // Initialize value on selection change
                            Connections {
                                target: window
                                function onActiveEditIndexChanged() {
                                    if (!posInput.activeFocus && monitorsModel.count > 0) {
                                        let mx = Math.round(monitorsModel.get(window.activeEditIndex).uiX / window.uiScale);
                                        let my = Math.round(monitorsModel.get(window.activeEditIndex).uiY / window.uiScale);
                                        posInput.text = mx + "x" + my;
                                    }
                            ...
                            onEditingFinished: {
                                if (monitorsModel.count > 0) {
                                    let parts = text.split("x");
                                    if (parts.length === 2) {
                                        let nx = parseInt(parts[0].trim());
                                        let ny = parseInt(parts[1].trim());
                                        if (!isNaN(nx) && !isNaN(ny)) {
                                            monitorsModel.setProperty(window.activeEditIndex, "uiX", nx * window.uiScale);
                                            monitorsModel.setProperty(window.activeEditIndex, "uiY", ny * window.uiScale);
                                            delayedLayoutUpdate.restart();
                                        }
                                    }
                                }
                    ...
                    Item { Layout.preferredHeight: window.s(10) }
