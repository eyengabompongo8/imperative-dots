...
compile_settings() {
    ...
    }' "${ENV_CONF}.tmp.$$" > "${ENV_CONF}.final.$$"
    mv -f "${ENV_CONF}.final.$$" "$ENV_CONF"
    rm -f "${ENV_CONF}.tmp.$$"
    ...
    sed -e "s|{{KB_LAYOUT}}|$LANG|g" \
        ...
        "$TMPL_DIR/settings.lua.template" > "${SETTINGS_CONF}.tmp.$$"
    mv -f "${SETTINGS_CONF}.tmp.$$" "$SETTINGS_CONF"

    # 3. Regenerate autostart.lua
    echo "Regenerating autostart.lua..."
    cp "$TMPL_DIR/autostart.lua.template" "${AUTOSTART_CONF}.tmp.$$"

    jq -r '.startup[]? | "    hl.exec_cmd(" + (.command | tojson) + ")"' "$SETTINGS_FILE" >> "${AUTOSTART_CONF}.tmp.$$"
...
# Zombie / duplicate prevention: ensure only one watcher daemon runs
for pid in $(pgrep -f "settings_watcher.sh"); do
